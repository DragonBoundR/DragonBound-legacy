...
router.get("/", function (req, res) {
  req.db.connection.getConnection().then((conn) => {
    conn
      .query(
        "SELECT u.rank, u.game_id, u.IdAcc, b.razon, b.date FROM users u INNER JOIN banned b ON b.UserId = u.IdAcc"
      )
      .then((rows4) => {
        conn.release();
        if (rows4[0].length > 0) {
          ...
          for (i = 0; i < rows_x2.length; i++) {
            if (rows_x2[i].date === "Forever") {
              ...
              date_state =
                '<td class="days">' +
                DateBan(parseInt(rows_x2[i].date)) +
                "</td>";
            }
            texto_test +=
              "<tr><td>" +
              ...
              "</td>" +
              date_state +
              '<td><img class="img_rank" src="/static/banned_ranks/' +
              rows_x2[i].rank +
              '.gif" ></td></tr>';
          }
          res.render("desban", {
