...
router.get("/", auth, csrfProtection, function (req, res) {
  ...
  req.db.connection.getConnection().then((conn) => {
    conn
      .query("SELECT game_id, rank, gm, IdAcc FROM users WHERE IdAcc = ?", [
        acc_id,
      ])
      .then((rows4) => {
        conn.release();
        if (rows4[0].length > 0) {
          ...
          if (
            rows_x2[0].IdAcc ===
            1 /* || rows_x2[0].IdAcc === 2 || rows_x2[0].IdAcc === 6*/
          ) {
            ...
            for (var i = 0; i < db.length; i++) {
              ...
              if (_stats.min_rank === 26) {
                ...
                rows_table_results +=
                  ...
                  "</th><td>" +
                  n[5] +
                  '</td><td align="center" valign="middle"><div class="img_ava" id="MyPlayer' +
                  n[0] +
                  '"></div></td></tr>';

                ava_script +=
                  "<script>PlayerAnim($('#MyPlayer" +
                  n[0] +
                  "'), [{file: '" +
                  n[4] +
                  ".png',type: '" +
            ...
            Logger.info(
              "GM " +
                rows_x2[0].game_id +
                " has entered the web of special avatars"
            );
            res.render("avatars", {
              //rows_x2[0].
            ...
            Logger.info(
              "The user " +
                rows_x2[0].game_id +
                " wanted to enter the web of avatars where he is not allowed to enter"
            );
            res.render("avatars", {
              //rows_x2[0].
              avatars: "",
