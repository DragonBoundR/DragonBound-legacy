...
const botLibrary = require('./FrogEngine/computer');
var fs = require('fs');
var db = require('./data');
...
module.exports = class Account {
	...
	unlocknewbot(){
        ...
        if(new_bot_id < botLibrary.COMPUTER_PLAYER.length){
            var bot = botLibrary.getInfo(new_bot_id);
						...
						if (data.banned === 1) {
							...
							self.gameserver.checkAccountOnlineAndClose(data.user_id, function () {
                                ...
                                if (self.player.rank >= 0) {
                                    ...
                                    else if (self.player.gp >= 427505) { next_rank = 24; gift_rank = 9356; cash = 3000} else {}
			...
			case Types.CLIENT_OPCODE.tournament_start_game:
				{
					...
					if (self.player.tournament_wait_game == 1) {
						...
						var Santa_Claus_Head = 120;
						var Santa_Claus_Body = 119;
						var Ava_Send = 0;
						//if (self.location_type === Types.LOCATION.CHANNEL && self.player.tournament_wait_game == 1 && self.gameserver.id === 6 || self.location_type === Types.LOCATION.CHANNEL && self.player.tournament_wait_game == 1 && self.gameserver.name === "Bunge.") {
                        if (self.gameserver.name === 'Holiday' && self.player.tournament_wait_game == 1 && self.player.gm_probability === random_probability_holiday/* || self.player.user_id === 1*/) {
                                ...
                                self.gameserver.getRoomById(id, function (room) {
                                    if (room) {
                                        var bot_data = {user_id: id_bot,reg_id: id_bot,game_id: 'Halloween',rank: 26,gp: 0,gold: 0,cash: 0,gender: "m",photo_url: "1234",ahead: 17,abody: 31,aeyes: 0,aflag: 0,abackground: 0,aforeground: 0,is_muted: 0,guild: '',guild_id: 0,guild_job: 0};
                                        var plx = new Player(bot_data);
                                        plx.is_bot = 1;
                                        plx.is_ready = 1;
                                        plx.position = 1;
                                        plx.mobile = Types.MOBILE.ADUKA;
                                        plx.scores_lose = 1;
                                        let acc = new Bot(plx);
                                        acc.user_id = id_bot;
                                        acc.gameserver = self.gameserver;
													...
													if (teamm === playerdbb.team) {
                                                        ...
                                                        if (accountdbb.scores_lose !== 1) {
                                                            ...
                                                            if (playerdbb.gender === 'm') {
                                                                //Ava_Send = 812;
                                                                Santa_Claus_Head = 120;
                                                                Santa_Claus_Body = 119;
                                                            } else {
                                                                //Ava_Send = 812;
                                                                Santa_Claus_Head = 120;
                                                                Santa_Claus_Body = 119;
															...
															self.gameserver.db.getUserAvatarsByIdAccANDaId(playerdbb.user_id, Ava_Send).then(function (rowss) {
																...
																accountdbb.gameserver.pushBroadcast(new Message.chatResponse(self, plx.game_id+" sent gift -> "+name_ava_gift_hd+" [Head] to -> "+playerdbb.game_id, Types.CHAT_TYPE.GOLD));
                                                                accountdbb.gameserver.pushBroadcast(new Message.chatResponse(self, plx.game_id+" sent gift -> "+name_ava_gift_bd+" [Body] to -> "+playerdbb.game_id, Types.CHAT_TYPE.GOLD));
																
																self.gameserver.chathistory.push([plx.game_id+" sent gift -> "+name_ava_gift_hd+" [Head] to -> "+playerdbb.game_id,'',Types.CHAT_TYPE.GOLD,'']);
                                                                self.gameserver.chathistory.push([plx.game_id+" sent gift -> "+name_ava_gift_bd+" [Body] to -> "+playerdbb.game_id,'',Types.CHAT_TYPE.GOLD,'']);
															});
	...
	onCreateBot(
        ...
        })
        {
            var self = this;
            let id = bot_id;
            let info_bot = botLibrary.getInfo(bot_id);
            let bot_data = {
                user_id: bot_id,
				        bot_id:bot_id,
                reg_id: 0,
                game_id: info_bot.name,
                rank: info_bot.rank,
                gp: 0,
                gold: 0,
                cash: 0,
                gender: "m",
                photo_url: "",
                ahead: info_bot.avatar.h,
                abody: info_bot.avatar.b,
                aeyes: info_bot.avatar.g,
                aflag: info_bot.avatar.f,
                abackground: 0,
                aforeground: 0,
                is_bot: true,
            };
