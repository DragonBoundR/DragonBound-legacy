...
module.exports = class DataBase {
    constructor() {
        ...
        /*if (process.env.vps == '1') {
            self.host = 'localhost';
            self.user = 'root';
            self.password = '$#%batfunproject2020$35';
            self.database = 'game';
    ...
    updateAct(a) {
        ...
        return new Promise(function(resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET activate = 2 WHERE game_id = ?', [a]).then(rows => {
                    conn.release();
    ...
    getUserByIdAcc(id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT rank, game_id, gender, country, banned FROM users where IdAcc = ?', [id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
...
conn.release();
    ...
    getLuckyEggs(user_id,cb){
        var self = this;
            self.connection.getConnection().then(conn => {
                conn.query('SELECT amount FROM user_avatars WHERE UserId = ?  AND aId = 2319 LIMIT 1', [user_id])
                    .then(rows => {
                        conn.release();
	...
	getMegaphone(user_id,cb){
        var self = this;
            self.connection.getConnection().then(conn => {
                conn.query('SELECT amount FROM user_avatars WHERE UserId = ?  AND aId = 894 LIMIT 1', [user_id])
                    .then(rows => {
                        conn.release();
                        cb(rows[0].length > 0 ? rows[0][0].amount : 0);
                    });
    ...
    removeLuckyEgg(user_id,cb){
        var self = this;
            self.connection.getConnection().then(conn => {
                conn.query('DELETE FROM user_avatars WHERE UserId = ?  AND aId = 2319 LIMIT 1', [user_id])
                    .then(rows => {
                        conn.release();
                        cb();
                    });
	...
	removeMegaphone(user_id,cb){
        var self = this;
            self.connection.getConnection().then(conn => {
                conn.query('DELETE FROM user_avatars WHERE UserId = ?  AND aId = 894 LIMIT 1', [user_id])
                    .then(rows => {
                        conn.release();
						cb();
                    });
    ...
    getUserAvatarsByIdAcc(id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT Id, UserId, aId, type, is_cash, is_gift, amount, expire_time FROM user_avatars WHERE UserId = ?  AND remove_ava = 0', [id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
    ...
    deleteAvatarById(Id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE user_avatars SET remove_ava = 1 WHERE Id = ?', [Id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    putPinCode(pin, seller, gm, gm_id, rode, state) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('INSERT into pin_code SET pin = ?, seller = ?, gm = ?, gm_id = ?, used_by = 0, rode = ?, state = ?, date_time = 0', [pin, seller, gm, gm_id, rode, state])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0)
                            return resolve(rows);
    ...
    updateTimeByEventServer(date, time, tipo, server_event, id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE event_game SET date = ?, time = ?, tipo = ?, server_check = ? WHERE Server_Id = ?', [date, time, tipo, server_event, id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
	...
	viewsServer1(server) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE servidores SET minUser = minUser + 1 WHERE Id = ?', [server])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    getMyReplayRoomGameByLetters(replay_room) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT replay_id, view_code, avatars_ids, code_rmd FROM view_replay WHERE view_code = ?', [replay_room])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
    ...
    putMyReplayRoomGame(view_code, avatars_ids, code_rmd/*, replay_code*/) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('INSERT INTO view_replay SET view_code = ?, avatars_ids = ?, code_rmd = ?', [view_code, avatars_ids, code_rmd/*, replay_code*/])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0)
                            return resolve(rows);
	...
	getBoy(UserId, aId) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT Id, aId, type, expire, is_cash, is_gift, amount, expire_time FROM user_avatars WHERE LOWER(`UserId`) = ? AND LOWER(aId) = ?', [UserId, aId])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
    ...
    updateResetRankingPrix(punts) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET prixw = 0 WHERE prixw != ?', [punts])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
	...
	deleteAvatarByUserId(user_id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('DELETE FROM user_avatars WHERE UserId = ?', [user_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    updateResetRankingRiD(last_reset_rankings, next_reset_rankings, RiD) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE resets_rankings SET last_reset_rankings = ?, next_reset_rankings = ? WHERE r = ?', [last_reset_rankings, next_reset_rankings, RiD])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    updateResetGPsGuilds(GPsGuild) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE guild SET members = 0 WHERE members >= ? && rank < 26', [GPsGuild])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    updateGPsRankingSemanalByIdAcc(gp, inicio) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET ranking_semanal = ? WHERE ranking_semanal >= ?', [gp, inicio])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    getLastRecord(rank) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT u.game_id, u.gp, u.rank, u.IdAcc, g.Name FROM users u LEFT JOIN guild_member m ON m.UserId = u.IdAcc LEFT JOIN guild g ON g.Id = m.Id WHERE u.rank = ? ORDER BY u.gp ASC limit 0, 1', [rank])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
    ...
    getAvatarExpireExItem(user_id, avatar_id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT u.IdAcc, u.game_id, u.power_user, v.aId FROM users u INNER JOIN user_avatars v ON u.IdAcc = v.UserId WHERE u.IdAcc = ? AND v.aId = ?', [user_id, avatar_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
	...
	updateNoWinBonusByIdAcc(date, user_id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE accounts SET no_win_bonus = ? WHERE Id = ?', [date, user_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
                ...
                .then(function (rows) {
                    if (rows[0].length > 0) {
                        ...
                        for (var xm in res0) {
                            ...
                            lstAvatars.push([res0[xm].Id, res0[xm].aId, equip, res0[xm].type, res0[xm].is_cash, res0[xm].is_gift, res0[xm].expire_time, res0[xm].amount]);
                        }
    ...
    updateMegaphone(cant, user_id) {
        ...
        return new Promise(function(resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET megaphones = megaphones + ? WHERE IdAcc = ?', [cant, user_id]).then(rows => {
                    conn.release();
                    if (rows[0].affectedRows > 0 || rows[0].changedRows > 0) return resolve(rows);
                    else return reject();
                });
            });
        });
...
updatelucky(cant, user_id) {
        ...
        return new Promise(function(resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET lucky_egg_sec_left = lucky_egg_sec_left + ? WHERE IdAcc = ?', [cant, user_id]).then(rows => {
                    conn.release();
                    if (rows[0].affectedRows > 0 || rows[0].changedRows > 0) return resolve(rows);
                    else return reject();
                });
...
UpdateAvatarAmountBuggle(UserId) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('UPDATE user_avatars SET amount = amount - 1 WHERE UserId = ? AND aId = ?', [UserId, 894]).then(rows => {
conn.release();
...
UpdateAvatarAmountlucky(UserId) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('UPDATE user_avatars SET amount = amount - 1 WHERE UserId = ? AND aId = ?', [UserId, 2319]).then(rows => {
conn.release();
                if (rows[0].length > 0) return resolve(rows);
                else return reject();
            });
    ...
    dontExistGuildName(name) {
     ...
     updateNewVersionName(nombrenuevo, nombrewher) {
        ...
        return new Promise(function(resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET game_id = ? WHERE game_id = ?', [nombrenuevo, nombrewher]).then(rows => {
                    conn.release();
                    if (rows[0].length > 0) return reject();
                    else return resolve();
                });
    ...
    putGuild(name) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query("INSERT INTO guild SET Name = ?, points = 0, members = 0, rank = 0, about = 'Bienvenidos'", [name])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0)
                            return resolve(rows);
    ...
    updateGoldGpWinLossByIdAcc(win_gold, win_gp, is_win, is_loss, win_gp2, user_id) {
        var self = this;
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('INSERT INTO games (user_id,gp,win,freg) VALUES (?,?,?,?)', [user_id,win_gp,is_win,Date.now()])
                    .then(rows => {
                        conn.release();
                    });
            });
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET gold = gold + ?, gp = gp + ?, win = win + ?, loss = loss + ?, ranking_semanal = ranking_semanal + ? WHERE IdAcc = ?', [win_gold, win_gp, is_win, is_loss, win_gp2, user_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    updateUser(account, callback) {
        ...
        var data = {};
        return new Promise(function (resolve, reject) {
            self.updateGoldGpWinLossByIdAcc(account.player.win_gold, account.player.win_gp, account.player.is_win, account.player.is_loss, account.player.win_gp, account.user_id)
                .then(function (rows) {
    ...
    sendGift(user_id,item_id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query("INSERT INTO user_avatars (UserId, aId, type, is_cash, is_gift, amount, expire_time) VALUES (?, ?, '0', '0', '1', '0', '0')", [user_id,item_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    getEventLogByIdAcc(user_id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT Event1, Event2 FROM users WHERE IdAcc = ?', [user_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
...
eventS(type, time, id) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if(type == 1){
            self.connection.getConnection().then(conn => {
            conn.query('UPDATE users SET Event1 = ? WHERE IdAcc = ?', [time, id]).then(rows => {
            conn.release();
                if (rows[0].length > 0) return resolve(rows);
                else return reject();
            });
            ...
            self.connection.getConnection().then(conn => {
            conn.query('UPDATE users SET Event2 = ? WHERE IdAcc = ?', [time, id]).then(rows => {
            conn.release();
                if (rows[0].length > 0) return resolve(rows);
                else return reject();
            });
    ...
    updatePowerUser(power_user, user_id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET power_user = ? WHERE IdAcc = ?', [power_user, user_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    updateLuckyEggLeft(user_id,lucky_egg_sec_left){
        this.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET lucky_egg_sec_left = ? WHERE IdAcc = ?', [lucky_egg_sec_left, user_id]).then(rows => {
                    conn.release();
                    if (!rows[0].affectedRows > 0) console.log("No se ha podido actualizar el lucky_egg_sec_left del usuario ",user_id);
                   /* else return reject();*/
                });
        });
    }
    reduceLuckyEgg(user_id,cb){
            this.connection.getConnection().then(conn => {
                conn.query('UPDATE user_avatars SET amount = amount-1 WHERE UserId = ? AND aId = 2319', [user_id])
                    .then(rows => {
                        cb();
                        conn.release();
                    });
	...
	reduceMegaphone(user_id,cb){
            this.connection.getConnection().then(conn => {
                conn.query('UPDATE user_avatars SET amount = amount-1 WHERE UserId = ? AND aId = 894', [user_id])
                    .then(rows => {
                        cb();
                        conn.release();
                    });
    ...
    sendRank(rank, user_id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET rank = rank + ? WHERE IdAcc = ?', [rank, user_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    sendGold(gold, user_id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE users SET gold = gold + ? WHERE IdAcc = ?', [gold, user_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
                            return resolve(rows);
    ...
    getAccountByFBId(id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('SELECT * FROM accounts where facebook_id = ?', [id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
    ...
    getEmail(mail,id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
                conn.query('UPDATE accounts SET Email = ? WHERE Id = ?', [mail,id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
    ...
    getExchange(game_id) {
        ...
        return new Promise(function (resolve, reject) {
            self.connection.getConnection().then(conn => {
				conn.query('SELECT u.IdAcc, u.game_id, a.email, a.password, u.name_changes, u.cash, u.gold, u.country, u.rank, u.gp, u.bg_url, u.photo_url FROM users u LEFT JOIN accounts a on u.IdAcc=a.id WHERE IdAcc = ?',[game_id])
                    .then(rows => {
                        conn.release();
                        if (rows[0].length > 0)
                            return resolve(rows);
...
DeleteAvatarX(UserId, type) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            ...
            if(type == 1){
                    conn.query('UPDATE users SET power_user = 0 WHERE IdAcc = ?', [UserId]).then(rows => {
                    conn.release();
                    if (rows[0].length > 0) return resolve(rows);
                    else return reject();
                    });
            }else if(type == 2){
                    conn.query('UPDATE users SET plus10gp = 0 WHERE IdAcc = ?', [UserId]).then(rows => {
                    conn.release();
                    if (rows[0].length > 0) return resolve(rows);
                    else return reject();
                    });
            }else if(type == 3){
                    conn.query('UPDATE users SET map_pack = 0 WHERE IdAcc = ?', [UserId]).then(rows => {
                    conn.release();
                    if (rows[0].length > 0) return resolve(rows);
                    else return reject();
                    });
            }else if(type == 4){
                    conn.query('UPDATE users SET lucky_egg = 0 WHERE IdAcc = ?', [UserId]).then(rows => {
                    conn.release();
                    if (rows[0].length > 0) return resolve(rows);
                    else return reject();
                    });
            }else if(type == 5){
                    conn.query('UPDATE users SET megaphones = 0 WHERE IdAcc = ?', [UserId]).then(rows => {
                    conn.release();
                    if (rows[0].length > 0) return resolve(rows);
                    else return reject();
                    });
            }else if(type == 6){
                    conn.query('UPDATE users SET mobile_fox = 0 WHERE IdAcc = ?', [UserId]).then(rows => {
                    conn.release();
                    if (rows[0].length > 0) return resolve(rows);
                    else return reject();
                    });
...
selectItemDetail(IdItem) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('select * from user_avatars WHERE Id = ?', [IdItem]).then(rows => {
conn.release();
                if (rows[0].length > 0) return resolve(rows);
                else return reject();
            });
...
defaultAvatars(myid, a, b) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('UPDATE user_avatar_equiped SET head = ? , body = ?, eyes = 0, flag = 0, background = 0, foreground = 0 WHERE Id = ?', [a, b, myid]).then(rows => {
conn.release();
                if (rows[0].length > 0) return resolve(rows);
                else return reject();
            });
...
verifyUser(game_id) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('SELECT game_id FROM users where game_id = ?', [game_id]).then(rows => {
conn.release();
                if (rows[0].length > 0){
                    return resolve(rows); 
                }
...
verifyMail(mail) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('SELECT count(*) as verdad FROM users where email = ?', [mail]).then(rows => {
conn.release();
                if (rows[0].length > 0){
                    return resolve(rows); 
                }
...
tokenVerify(a,b) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('SELECT count(*) as verdad, (select game_id from users where IdAcc = ?) as nick, idAcc FROM tokenMail where token = ? AND idAcc = ?', [b,a,b]).then(rows => {
conn.release();
                if (rows[0].length > 0){
                    return resolve(rows); 
                }
...
verificarNombre(a) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('SELECT count(*) as name1 from users where game_id = ?', [a]).then(rows => {
conn.release();
                if (rows[0].length > 0){
                    return resolve(rows); 
                }
...
verificarNombre2(a) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('SELECT count(*) as name2 from users where name2 = ?', [a]).then(rows => {
conn.release();
                if (rows[0].length > 0){
                    return resolve(rows); 
                }
...
verificarNombre3(a) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('SELECT count(*) as T from accounts where name2 = ?', [a]).then(rows => {
conn.release();
                if (rows[0].length > 0){
                    return resolve(rows); 
                }
...
tokenDelete(a) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('DELETE FROM tokenmail WHERE idAcc = ?', [a]).then(rows => {
conn.release();
                if (rows[0].length > 0){
                    return resolve(rows); 
                }
...
tokenACTV(a) {
    ...
    return new Promise(function(resolve, reject) {
        self.connection.getConnection().then(conn => {
            conn.query('UPDATE users SET activate = 2 WHERE IdAcc = ?', [a]).then(rows => {
                conn.release();
                if (rows[0].length > 0) return resolve(rows);
                else return reject();
            });
