...
module.exports = class Room {
        ...
        if (account.player.is_bot === 0) {
          self.updatePosition().then(function () {
            self.gameserver.pushToRoom(
              self.id,
              new Message.changedTeam(account, self)
            );
          });
        }
      }
  ...
  chat(account, msj, team) {
    ...
    //}//
    if (
      account.player.is_muted === true ||
      account.player.is_muted >= Date.now()
    ) {
      account.sendMessage(
    ...
    if (account.player.power_user === 1) type = Types.CHAT_TYPE.POWER_USER;

    if (account.player.gm === 1) type = Types.CHAT_TYPE.GM;

    if (
      (account.player.rank === 28 && account.player.gm === 1) ||
      (account.player.rank === 29 && account.player.gm === 1) ||
      (account.player.rank === 30 && account.player.gm === 1)
    )
    ...
    if (!this.watchers[account.user_id])
      if (team === 1) {
        ...
        self.gameserver.pushBroadcastChat(
          new Message.chatResponse(account, msj, type),
          self
        );
      }
  ...
  removeWatcher(account, fd = false) {
    ...
    if (account.player.gm !== 1)
      this.gameserver.pushToRoom(
        this.id,
        new Message.watcherLeft(account.user_id),
        null
      );
    if (!fd) {
      account.room = null;
  ...
  joinWatcher(account) {
    ...
    if (account.player.gm !== 1)
      this.gameserver.pushToRoom(
        this.id,
        new Message.watcherJoined(account),
        account.user_id
      );
  }
  joinPlayer(account) {
    var self = this;
    if (account.room != null) return false;
    if (
      this.game_mode === Types.GAME_MODE.BOSS ||
      this.search_team_room === 1
    ) {
      if (account.player.is_bot === 0) {
    ...
    if (self.gameserver.server_subtype !== 3) self.gameserver.sendRooms();
    self.canremove = true;
  }
  ...
  changeTeam(account) {
    ...
    if (
      this.game_mode === Types.GAME_MODE.BOSS ||
      this.status == Types.ROOM_STATUS.PLAYING
    ) {
      return null;
    }
    let player = account.player;
  ...
  gameStart(account) {
    ...
    if (
      ...
      if (
        (self.player_count > 0 &&
          self.game_mode === Types.GAME_MODE.BOSS &&
          self.checkReady()) ||
        (self.player_count > 1 &&
          self.game_mode === Types.GAME_MODE.NORMAL &&
          self.team_a_count === self.team_b_count &&
          self.checkReady()) ||
        (self.player_count > 1 &&
          self.game_mode === Types.GAME_MODE.SAME &&
          self.team_a_count === self.team_b_count &&
          self.checkReady()) ||
        (self.player_count > 1 &&
          self.game_mode === Types.GAME_MODE.SCORE &&
          self.team_a_count === self.team_b_count &&
          self.checkReady()) ||
        (self.player_count > 1 &&
          self.game_mode === Types.GAME_MODE.TAG &&
          self.team_a_count === self.team_b_count &&
          self.checkReady())
      ) {
            ...
            self.forPlayerB(function (account_b) {
              if (typeof account_b !== "undefined") {
                if (account_a.player.no_win_bonus_accounts !== "{}") {
                  if (
                    typeof account_a.player.no_win_bonus_accounts[
                      parseInt(account_b.player.user_id)
                    ] !== "undefined"
                  ) {
                    if (
                      account_b.player.user_id ===
                      account_a.player.no_win_bonus_accounts[
                        parseInt(account_b.player.user_id)
                      ].user_id
                    ) {
                      if (
                        ...
                        delete account_a.player.no_win_bonus_accounts[
                          parseInt(account_b.player.user_id)
                        ];
                        self.gameserver.db
                          .updateNoWinBonusByIdAcc(
                            JSON.stringify(
                              account_a.player.no_win_bonus_accounts
                            ),
                            account_a.player.user_id
                          )
                          .then(function (data) {
                            if (data.error_mysql || data.error_querry) {
                            } else {
                              account_a.player.no_win_bonus_players =
                                account_a.player.no_win_bonus_accounts;
                              self.gameserver.pushToRoom(
                                self.id,
                                new Message.roomState(self)
                              );
                              self.gameserver.pushToRoom(
                                self.id,
                                new Message.roomPlayers(self),
                                null
                              );
                              account_a.sendMessage(
                                new Message.loginResponse(account_a)
                              );
                              Logger.debug(
                                "Up delete No Win Bonus Users: " +
                                  JSON.stringify(
                                    account_a.player.no_win_bonus_accounts
                                  )
                              );
            ...
            self.forPlayerA(function (account_a) {
              if (typeof account_a !== "undefined") {
                if (account_b.player.no_win_bonus_accounts !== "{}") {
                  if (
                    typeof account_b.player.no_win_bonus_accounts[
                      parseInt(account_a.player.user_id)
                    ] !== "undefined"
                  ) {
                    if (
                      account_a.player.user_id ===
                      account_b.player.no_win_bonus_accounts[
                        parseInt(account_a.player.user_id)
                      ].user_id
                    ) {
                      if (
                        ...
                        delete account_b.player.no_win_bonus_accounts[
                          parseInt(account_a.player.user_id)
                        ];
                        self.gameserver.db
                          .updateNoWinBonusByIdAcc(
                          ...
                          .then(function (data) {
                            if (data.error_mysql || data.error_querry) {
                              ...
                              account_b.player.no_win_bonus_players =
                                account_b.player.no_win_bonus_accounts;
                              self.gameserver.pushToRoom(
                                self.id,
                                new Message.roomState(self)
                              ...
                              account_b.sendMessage(
                                new Message.loginResponse(account_b)
                              );
                              Logger.debug(
                                "Up delete No Win Bonus Users: " +
                                  JSON.stringify(
              ...
              if (typeof account !== "undefined") {
                if (account.player.mobile === Types.MOBILE.RANDOM) {
                  account.player.mobile =
                    Types.MOBILE_R[getRndInteger(0, Types.MOBILE_R.length - 1)];
                }
                if (self.game_mode === Types.GAME_MODE.TAG) {
                  ...
                  account.send([
                    Types.SERVER_OPCODE.game_mode_gb_tag,
                    random_number,
                  ]);
                }
                ...
                if (self.is_avatars_on === 0) {
                  ...
                  if (
                    account.player.mobile !== Types.MOBILE.MAGE ||
                    account.player.mobile !== Types.MOBILE.LIGHTNING ||
                    account.player.mobile !== Types.MOBILE.ASATE ||
                    account.player.mobile !== Types.MOBILE.JD
                  ) {
                    account.player.shield = 0;
                    account.player.shield_regen = 0;
                  }
                ...
                self.win_team_gp =
                  self.team_b_count == 1
                    ...
                    : 0;
                if (
                  self.gameserver.evento200 === true ||
                  self.event_game_room === 1
                )
                  self.win_team_gp *= 2;
                ...
                self.win_team_gpb =
                  self.team_bots_count == 1
                    ? 29
                    : self.team_bots_count == 2
                    ? 42
                    : self.team_bots_count == 3
                    ? 52
                    : self.team_bots_count == 4
                    ? 62
                    : 0;
                if (
                  self.gameserver.evento200 === true ||
          ...
          self.game.onGameEnd(function (team) {
            ...
            self.forPlayers(function (account) {
              if (typeof account !== "undefined") {
                ...
                if (team === player.team) {
                  ...
                  if (self.event_game_room === 1) player.is_loss = 1;
                  else player.is_win = 1;
                  ...
                  self.forPlayers(function (account_loss) {
                    if (typeof account_loss !== "undefined") {
                      if (team != account_loss.player.team) {
                        if (
                          typeof player.no_win_bonus_players[
                            parseInt(account_loss.player.user_id)
                          ] !== "undefined"
                        ) {
                          if (
                            player.no_win_bonus_players[
                              parseInt(account_loss.player.user_id)
                            ].loss >= 99999
                          ) {
                            if (
                              ...
                              self.gameserver.db
                                .updateNoWinBonusByIdAcc(
                                  JSON.stringify(player.no_win_bonus_players),
                                  player.user_id
                                )
                                .then(function (data) {
                                  ...
                                  if (data.error_mysql || data.error_querry) {
                                    ...
                                    account.sendMessage(
                                      new Message.loginResponse(account)
                                    );
                                    Logger.normal(
                                      "Up No Win Bonus Users: " +
                                        JSON.stringify(
                                          player.no_win_bonus_accounts
                                        )
                                    );
                  ...
                  if (self.gameserver.server_subtype === 3) {
                    ...
                    if (self.gameserver.name === "Prix") {
                      if (
                        ...
                        player.punts_prix_user = parseInt(
                          player.punts_prix_user + 1
                        );
                      }
                      account.sendMessage(new Message.loginResponse(account));
                      self.gameserver.db
                        .updatePrix(player.punts_prix_user, account)
                        .then(function (data) {
                          if (data.error_mysql || data.error_querry) {
                          } else {
                    ...
                    if (
                      ...
                      player.guild !== "" &&
                      info_prix.players === 7 &&
                      player.tournament_start_time_server <= Date.now() &&
                      player.tournament_end_time_server >= Date.now()
                    ) {
                      player.guild_score += 1;
                      self.gameserver.db.updateGuildPrixById(
                        player.guild_score,
                        player.guild_id
                      );
                    }
                  ...
                  if (self.event_game_room === 1) player.is_win = 1;
                  else player.is_loss = 1;
                  account.saveWinDB(self.power == 1 ? true : false);
                  if (self.gameserver.server_subtype === 3) {
                    if (self.gameserver.name === "Holiday") {
            ...
            self.forPlayers(function (account_screen) {
              if (typeof account_screen !== "undefined") {
                account_screen.player.screenshot = JSON.stringify([
                  1,
                  Date.now(),
                  self.gameserver.id,
                  self.id,
                  ...
                  self.max_wind,
                  team,
                  self.game.turns_pass,
                  date_my_info_player,
                ]);
                account_screen.player.code_screenshot_random =
                  screenshot_letters(8);
              }
            });
            self.game = null;
            if (self.player_count < self.max_players)
              ...
              self.status = Types.ROOM_STATUS.FULL;
            else self.status = Types.ROOM_STATUS.PLAYING;
            if (self.game_mode === Types.GAME_MODE.BOSS) self.map = -1;
            if (self.gameserver.server_subtype !== 3)
              self.gameserver.sendRooms();
                  ...
                  if (players_server_event.player.is_bot === 0) {
                    setTimeout(function () {
                      if (
                        ...
                        self.gameserver.getRoomById(
                          parseInt(
                            players_server_event.player.team_tournament_room
                          ),
                          function (room_server_event) {
                            if (room_server_event) {
                              setTimeout(function () {
                                room_server_event.joinPlayer(
                                  players_server_event
                                );
                                room_server_event.team_tournament_game = 0;
                                room_server_event.room_tournament_playing = 0;
                                players_server_event.player.room_number =
                                  room_server_event.id;
                                players_server_event.player.team_tournament_room = 0;
                                if (
                                  players_server_event.player
                                    .user_master_room === 1
                                ) {
                                  players_server_event.player.is_master = 1;
                                }
                                players_server_event.location_type =
                                  Types.LOCATION.ROOM;
                        ...
                        players_server_event.sendMessage(
                          new Message.loginResponse(players_server_event)
                        );
                          ...
                          self.gameserver.forEachAccount(function (accounttp) {
                            if (
                              ...
                              if (self.gameserver.evento200 === true) {
                                var w = self.gameserver.SecondsToString(
  ...
  forTurnList(turnList, callback) {
    ...
    for (let i = 0; i < turnList.length; i++) {
      ...
      let account = self.gameserver.getAccountById(playerId)
        ? self.gameserver.getAccountById(playerId)
        : self.gameserver.getBotById(playerId);
      if (account) {
        returnList.push(
          self.formatTurnList({ lastturn: lastturn[i], account: account })
        );
      }
    }
    callback(returnList);
  ...
  checkReady() {
    ...
    self.forPlayers(function (account) {
      if (typeof account === "undefined") {
        ...
        if (
          (account.player !== null) & (account.player.is_ready === 0) &&
          account.player.is_master !== 1
        ) {
          canstart = false;
        }
      }
  ...
  getOnePlayer() {
    ...
    return new Promise(function (accept, reject) {
      for (let id in self.team_a) {
        ...
        if (typeof pl_teama === "undefined" || pl_teama === null) {
        } else {
          break;
        }
      }
      for (let id in self.team_b) {
        ...
        if (typeof pl_teamb === "undefined" || pl_teamb === null) {
        } else {
          break;
        }
      }
      if (pl_teama === null && pl_teamb === null) {
        ...
        if (
          pl_teama === null ||
          (pl_teamb !== null &&
            pl_teama.player.position > pl_teamb.player.position)
        ) {
          accept(pl_teamb);
        } else {
          accept(pl_teama);
  ...
  removePlayer(account) {
    ...
    try {
      ...
      self.turn_list.forEach(function (p, i) {
        if (account.user_id == p.user_id) self.turn_list.splice(i, 1);
      });
      if (self.game) {
        if (self.game.turns_pass <= 6 && self.player_count == 2) {
          ...
          self.gameserver.pushBroadcastChat(
            new Message.chatResponse(
              self,
              "Free Kill Detectado - No hay bonificación por victoria",
      ...
      if (self.status == Types.ROOM_STATUS.PLAYING) {
        if (
          self.game.turn_player == account.player.position &&
          self.player_count > 0
        ) {
        ...
        var message3 = new Message.chatResponse(
          self,
          "Player " + account.player.game_id + " left the room.",
          Types.CHAT_TYPE.SYSTEM
        ).serialize();
        var message4 = new Message.chatResponse(
          self,
          "Winning Bonus: Team A = %% GP, Team B = %% GP.",
          Types.CHAT_TYPE.SYSTEM
        ).serialize();
        //console.log(message);
        self.forPlayers(function (account) {
      ...
      if (
        ...
        self.gameserver.pushToRoom(
          self.id,
          new Message.playerLeft(account, self)
        );
          ...
          self.getOnePlayer().then(function (p) {
            if (account.user_id != p.user_id && self.found_master === false) {
              ...
              self.gameserver.pushBroadcastChat(
                new Message.chatResponse(
                  self,
                  "Master de la sala se transfirió a " + p.player.game_id,
                  Types.CHAT_TYPE.SYSTEM
                ),
                self
              );
  ...
  RoomUpdate(room_options = false) {
    var self = this;
    if (room_options) self.room_options = room_options;
    self.gameserver.pushToRoom(self.id, new Message.roomState(self));
