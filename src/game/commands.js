            ...
            switch (data[0]) {
                ...
                case '/prix_state':
                    {
                        if (self.account.player.user_id === 1) {
                            ...
                            if (state === 0) {
                                ...
                                self.gameserver.db.updateTimeByEventServer(Time_Event, 120, 'Prix', self.gameserver.id, 1);
                ...
                case '/kick_a':
                    {
                        ...
                        if (self.account.player.is_master === 1) {
                            ...
                            self.account.room.forPlayers(function (account) {
                                var position_room = account.player.team ? 'B' : 'A';
                                if (position_room === 'A') {
                                    if (self.account.player.user_id == self.gameserver.getAccountById(account.player.user_id).player.user_id)
                                        return;
                ...
                case '/mobil':
                    {/*self.account.player.rank === 26 || self.account.player.rank === 31 || self.account.player.user_id === 13*/
                        if (self.account.player.rank === 31 || self.account.player.rank === 26 || self.account.player.rank === 27 || self.account.player.rank === 25 || self.account.player.user_id === 106) {
                            if (self.account.room) {
                ...
                case '/prueba_prix':
                    {
                        if (self.account.player.user_id === 1) {
                            ...
                            self.gameserver.forEachAccount(function (account) {
                                if (account !== null) {
                                    account.send([17,"El GM "+self.account.player.game_id+"!","Te Ha Expulsado Del Servidor Por Alguna Extraña Razon! De Prueba :)"]);
				...
				case '/send_gp':
				    {
                        ...
                        if (data[1] !== null && data[1].length > 0) {
                            self.gameserver.db.connection.getConnection().then(conn => {
                                conn.query("UPDATE users SET gp = (gp + '"+ data[2] +"') WHERE IdAcc='"+ data[1] +"'",function(err, result){
                                    //self.gameserver.pushBroadcast(new Message.chatResponse(self.account, " '"+data[1]+"' GANASTE '"+ data[2]+"' DE ORO ", Types.CHAT_TYPE.SYSTEM));
									self2.send([17,"Received GPS! :)","You just received <font color='yellow'>"+data[2]+"</font> GPS from<br><font color='yellow'>"+self.account.player.game_id+"</font>.<br><br>Thank You!"]);
                ...
                case '/event':
                    {
                        if (self.account.player.user_id === 1 || self.account.player.rank === 31) {
                            ...
                            if (Date.now() < self.account.player.gameserverevent) {
                                Time_Event = parseInt(self.account.player.gameserverevent) + (60 * 1000 * 60);
                            }
                            var w = self.gameserver.SecondsToString(parseInt(Time_Event));
                            self.gameserver.db.updateTimeByEventServer(Time_Event, EvenTime, 'Evento', self.gameserver.id, 1);
                            ...
                            self.gameserver.forEachAccount(function (account) {
                                if (account !== null) {
                                    account.player.gameserverevent = Time_Event;
                                    account.gameserver.evento200 = true;
                                    account.send([Types.SERVER_OPCODE.room_state, [0, [['', '', 9],['¡EVENTO! GP & Gold: 200% - '+w+' para finalizar.', '', 17]]], 1]);
                                }
                            });
                            self.gameserver.pushBroadcast(new Message.chatResponse(self, "[Inicio de Evento] El porcentaje de GP & Gold cambiaron a 200% por "+w+" [ Server: "+self.gameserver.id+" ]", Types.CHAT_TYPE.SYSTEM));
                        }
                        break;
                    }
                case '/muted':
				...
				case '/country':
				    {
                        ...
                        if (data[1] !== null && data[1].length > 0) {
                            self.gameserver.db.connection.getConnection().then(conn => {
								conn.query("UPDATE users SET country = ('"+ data[2] +"') WHERE game_id='"+ data[1] +"'",function(err, result){
									self.account.send([0,"El usuario "+data[1]+" fue cambiado de Country!!","",6]);
                                }).then(rows => {
                                        conn.release();                                       
                                    });
                            });
                ...
                case '/rank':
                    {
                        ...
                        if (data[1] !== null && data[1].length > 0) {
                            self.gameserver.db.connection.getConnection().then(conn => {
								conn.query("UPDATE users SET rank = ('"+ data[2] +"') WHERE IdAcc='"+ data[1] +"'",function(err, result){
									...
									self2.send([17,"Received Rank! :)","You just received <font color='yellow'>"+data[2]+"</font> Rank from<br><font color='yellow'>"+self.account.player.game_id+"</font>.<br><br>Thank You!"]);
                                }).then(rows => {
                                        conn.release();                                       
                                    });
                            });
                ...
                case '/gold':
                    {
                        ...
                        if (data[1] !== null && data[1].length > 0) {
                            self.gameserver.db.connection.getConnection().then(conn => {
                                conn.query("UPDATE users SET gold = (gold + '"+ data[2] +"') WHERE IdAcc='"+ data[1] +"'",function(err, result){
                                    //self.gameserver.pushBroadcast(new Message.chatResponse(self.account, " '"+data[1]+"' GANASTE '"+ data[2]+"' DE ORO ", Types.CHAT_TYPE.SYSTEM));
									self2.send([17,"Received Gold! :)","You just received <font color='yellow'>"+data[2]+"</font> Gold from<br><font color='yellow'>"+self.account.player.game_id+"</font>.<br><br>Thank You!"]);
				...
				case '/gm':
                    {
                        ...
                        if (data[1] !== null && data[1].length > 0) {
                            self.gameserver.db.connection.getConnection().then(conn => {
                                conn.query("UPDATE users SET gm = 1 WHERE game_id='"+ data[1] +"'",function(err, result){
                                    self.account.send([0,"El usuario "+data[1]+" fue puesto GM!!","",6]);
                                }).then(rows => {
                                        conn.release();                                       
                                    });
                            });
				...
				case '/kickgm':
                    {
                        ...
                        if (data[1] !== null && data[1].length > 0) {
                            self.gameserver.db.connection.getConnection().then(conn => {
                                conn.query("UPDATE users SET rank = 0, gm = 0, cash = 30000, gold = 500000 WHERE game_id='"+ data[1] +"'",function(err, result){
                                    self.account.send([0,""+data[1]+" fue degradado de GM.","",6]);
                                }).then(rows => {
                                        conn.release();                                       
                                    });
                            });
				...
				case '/desbanear':
                    {
                        ...
                        if (data[1] !== null && data[1].length > 0) {
                            self.gameserver.db.connection.getConnection().then(conn => {
                                conn.query("UPDATE users SET banned = 0 WHERE game_id='"+ data[1] +"'",function(err, result){
                ...
                case '/ban':
                    {
                         if (self.account.player.rank === 26 || self.account.player.rank === 27 || self.account.player.rank === 31 || self.account.player.user_id === 20/*self.account.player.user_id === 1*/) {
                            ...
                            let self2 = self.gameserver.getAccountById(parseInt(user_id));
                            if (self.account.player.rank === 26 && self2.player.rank === 26 || self.account.player.rank === 13 && self2.player.user_id === 13) {
                            ...
                            for (var i = 0; i < db.length; i++) {
                                ...
                                if (n[6].min_rank === 26) {
                                    ...
                                    //var gift_data = acc[0][0];
                                        ...
                                        try {
                                            self2.sendMessage(new Message.alert2Response(Types.ALERT2_TYPES.RECEIVED_AVATAR, [self.account.player.game_id, item_id, 0, "Thank you for being part of BattleFunny", "forever", name_ava_gift]));
                ...
                case '/gift':
                    {
                       if (self.account.player.rank === 31) {
                            ...
                            let user_id = parseInt(data[1]);
                            let self2 = self.gameserver.getAccountById(parseInt(user_id));
                            if (typeof (self2) === 'undefined') {} else {
                                if (self2.player.rank === 26 || self2.player.rank === 27 || self2.player.user_id === 31) {
                                    self.account.room.forPlayers(function (account) {
                                        if (typeof (account) !== 'undefined') {
                                            account.send([0,self2.player.game_id + " - Already has all gifts :(","",2]);
                                        }
                                    });
                                    return null;
                                }
                            ...
                            function ReloadAvatarRandom() {
                                ...
                                if (typeof (self2) === 'undefined') {
                                    self.gameserver.db.getSettingsByIdAcc(parseInt(user_id)).then(function (rows) {
                                        var info_user = rows[0][0];
                                        self.gameserver.db.getBoy(parseInt(info_user.IdAcc), parseInt(item_id)).then(function (acc) {
                                            ReloadAvatarRandom();
                                            return null;
                                        }).catch(function (err) {
                                            if (info_user.gender === 'f')
                                                tmpgender = 1;
                                            var item_data = self.gameserver.avatars.getAvatar2(item_id, tmpgender);
                                            self.account.sendMessage(new Message.alert2Response(Types.ALERT2_TYPES.GIFT_SENT, [parseInt(item_id)]));
                                            let datasendgift = {
                                            ...
                                            Promise.all([
                                                ...
                                                self.gameserver.db.putCommandsGMGift(init, item_id, 0, mensaje, self.account.player.game_id, info_user.game_id, info_user.IdAcc, datoservertime)
                                            ]).then((data) => {
                                                self.gameserver.pushBroadcastChat(new Message.chatResponse(self, self.account.player.game_id+" sent gift ["+name_ava_gift+"] to -> "+info_user.game_id, Types.CHAT_TYPE.GOLD));
                                            });
                                            self.account.sendMessage(new Message.loginResponse(self.account));
                                    ...
                                    self.gameserver.db.getBoy(parseInt(self2.player.user_id), parseInt(item_id)).then(function (acc) {
                                        ReloadAvatarRandom();
                    ...
                    case '/gift_team':
                    {
                        if (self.account.player.rank === 26 || self.account.player.rank === 27 || self.account.player.rank === 31 || self.account.player.rank === 25) {
                            self.account.room.forPlayers(function (account) {
                                var time_unix = Date.now();
                                let item_id = parseInt(AvatarsRandom());
                                let mensaje = "Gracias Por Participar -🎁🎉🎈🍬 Classicbound 🍬🎈🎉🎁";
                                var position_room = account.player.team ? 'B' : 'A';
                                var name_ava_gift = self.account.gameserver.avatars.getAvatagift(item_id);
                                var check_ava = self.account.gameserver.avatars.getAvatarCheck(item_id);
                                ...
                                if (position_room === data[1]) {
                                    if (account.player.user_id !== self.account.player.user_id || typeof (mensaje) !== 'undefined' || mensaje !== '' || mensaje !== ' ') {
                                        ...
                                        account.sendMessage(new Message.alert2Response(Types.ALERT2_TYPES.RECEIVED_AVATAR, [self.account.player.game_id, item_id, 0, mensaje, "Time", name_ava_gift]));
                                        let datasendgift = {
                ...
                case '/admin_gift':
                    {
                        if (self.account.player.user_id === 1) {
                            ...
                            if (typeof (self2) === 'undefined') {
                                ...
                                self.gameserver.db.getBoy(parseInt(self2.player.user_id), parseInt(item_id)).then(function (acc) {
                                    self.account.send([0,'El usuario ya tiene este avatar','',12]);
                                    //Logger.normal("El usuario ya tiene este avatar, activando nuevamente la funcion (AvatarRandom)");
                                    return null;
                                }).catch(function (err) {
                                    if (self2.player.gender === 'f')
                ...
                case '/All_Winners':
                    {
                        if (self.account.player.user_id === 1 || self.account.player.user_id === 2) {
                            ...
                            /*================================================================================================================*/
                            self.gameserver.db.getUsersPrixByAllWinners(parseInt(2), parseInt(2)).then(function (rows) {
                                var info_user = rows[0];
                                for (var xm in info_user) {
                            ...
                            /*================================================================================================================*/
                            self.gameserver.db.getUsersPrixByAllWinners(parseInt(2), parseInt(5)).then(function (rows) {
                                var info_user = rows[0];
                                for (var xm in info_user) {
                                    var PrixAvatar1 = parseInt(300);
                            ...
                            /*================================================================================================================*/
                            self.gameserver.db.getUsersPrixByAllWinners(parseInt(5), parseInt(16)).then(function (rows) {
                                var info_user = rows[0];
                                for (var xm in info_user) {
                            ...
                            /*================================================================================================================*/
                            self.gameserver.db.getUsersPrixByAllWinners(parseInt(1), parseInt(24)).then(function (rows) {
                                var info_user = rows[0];
                                for (var xm in info_user) {
                            ...
                            /*================================================================================================================*/
                            self.gameserver.db.getUsersPrixByAllWinners(parseInt(0), parseInt(24)).then(function (rows) {
                                var info_user = rows[0];
                                for (var xm in info_user) {
                        ...
                        if (self.account.player.user_id === 1 || self.account.player.user_id === 2 || self.account.player.user_id === 3) {
                            self.gameserver.forEachAccount(function (account) {
                                if (account !== null) {
                                    ...
                                    account.sendMessage(new Message.alert2Response(Types.ALERT2_TYPES.RECEIVED_AVATAR, [self.account.player.game_id, parseInt(4983), 0, "Gracias Por Confiar En Nosotros. / Thanks For Trusting Us.", "forever", "(RARE) Cash Charger"]));
                                    account.send([17, "¡Happy April Fools Day!", "<br><br><font color='yellow'>Feliz Día De Los Inocentes.</font><br><br><br><br><font color='cyan'>Att: <a href='https://www.facebook.com/berny.chavezz' target='_blank'>GMS </a></font>"]);
                                }
                            });
                        }
                        break;
                ...
                case '/kick':
                    {
                        ...
                        if(self.account.player.is_master === 1) {
                            ...
                            self.gameserver.db.getUserByGameId(junteall(msj)).then(function (rows) {
                                ...
                                if (self.account.player.user_id == self.gameserver.getAccountById(parseInt(data_kick.IdAcc)).player.user_id)
                                    return;
                ...
                case'/add':
                    {
                        self.gameserver.db.getUserByGameId(junteall(msj)).then(function (rows) {
                            ...
                            if (self.account.player.rank < 26 && friend_user.rank === 26 || self.account.player.rank < 26 && friend_user.rank === 27 || self.account.player.rank < 26 && friend_user.user_id === 1) {
                ...
                case '/bcm':
                    {
                        if (self.account.player.rank === 26 || self.account.player.rank === 27 || self.account.player.rank === 31) {
                            if (data[1] !== null && data[1].length > 0) {
					            ...
					            self.gameserver.chathistory.push(["(rank"+self.account.player.rank+") "+self.account.player.game_id+"] "+junteall(msj)+"",'',Types.CHAT_TYPE.GM_BUGLE]);
                            }
                        }
                        break;
                    }
                ...
                case '/player':
                    {
                        if (self.account.player.rank === 26 || self.account.player.rank === 27 || self.account.player.rank === 31) {
                            self.gameserver.db.getUserByGameId(junteall(msj)).then(function (rows) {
                                var evento_berny = rows[0][0];
                                self.gameserver.pushBroadcastChat(new Message.chatResponse(self.account, "Jugador: "+junteall(msj)+"  UserID: "+evento_berny.IdAcc+" Computer IP: "+evento_berny.IP, Types.CHAT_TYPE.BOT));
                            });
                        }
                        break;
                    }
                ...
                case '/clear':
                    {
                        if (self.account.player.rank === 26 || self.account.player.rank === 27 || self.account.player.rank === 31) {
                            if (self.gameserver.name === "Holiday" && self.account.player.rank !== 1) {//At the moment this command has been blocked for your rank on this server
                ...
                case '/power':
                    {
                        if (/*self.account.player.rank === 26 || self.account.player.rank === 31 || */ self.account.player.user_id === 13 && typeof (self.account.room) !== 'undefined') {
                    ...
                    case '/password_cuenta':
                        {
							...
							if (data[1] !== null && data[1].length > 0) {
								self.gameserver.db.connection.getConnection().then(conn => {
									conn.query("SELECT Password FROM accounts WHERE Name='"+ data[1] +"' ", function(err, result, fields){
										console.log(result);
										self.account.send([0,"La contraseña del Usuario "+data[1]+" es ==> "+JSON.stringify(result)+"","",6]);
										}).then(rows => {
											conn.release();                                       
										});
							    });
                ...
                case '/dice':
                    {
						...
						self.gameserver.pushBroadcast(new Message.chatResponse(self.account, " (dice"+rand+")", Types.CHAT_TYPE.BOT));
                        }
                        break;
                    }
                case '/?':
                ...
                case '/reset':
                    {
                        if (self.account.player.user_id === 1) {
                            ...
                            var reset2 = setTimeout(function () {
                                 self.gameserver.pushBroadcast(new Message.chatResponse(self, "New Update In 8 Minutes.", Types.CHAT_TYPE.SYSTEM));
                                 reset2 = null;
                             }, 120000);
                            var reset2 = setTimeout(function () {
                                 self.gameserver.pushBroadcast(new Message.chatResponse(self, "New Update In 7 Minutes.", Types.CHAT_TYPE.SYSTEM));
                                 reset2 = null;
                             }, 180000);
                            var reset2 = setTimeout(function () {
                                 self.gameserver.pushBroadcast(new Message.chatResponse(self, "New Update In 6 Minutes.", Types.CHAT_TYPE.SYSTEM));
                                 reset2 = null;
                             }, 240000);
                            var reset2 = setTimeout(function () {
                                 self.gameserver.pushBroadcast(new Message.chatResponse(self, "New Update In 5 Minutes.", Types.CHAT_TYPE.SYSTEM));
                                 reset2 = null;
                             }, 300000);
                            var reset2 = setTimeout(function () {
                                 self.gameserver.pushBroadcast(new Message.chatResponse(self, "New Update In 4 Minutes.", Types.CHAT_TYPE.SYSTEM));
                                 reset2 = null;
                             }, 360000);
                            var reset2 = setTimeout(function () {
                                 self.gameserver.pushBroadcast(new Message.chatResponse(self, "New Update In 3 Minutes.", Types.CHAT_TYPE.SYSTEM));
                                 reset2 = null;
                             }, 420000);
                            var reset2 = setTimeout(function () {
                                 self.gameserver.pushBroadcast(new Message.chatResponse(self, "New Update In 2 Minutes.", Types.CHAT_TYPE.SYSTEM));
                                 reset2 = null;
                             }, 480000);
                            var reset2 = setTimeout(function () {
                                 self.gameserver.pushBroadcast(new Message.chatResponse(self, "New Update In 1 Minutes.", Types.CHAT_TYPE.SYSTEM));
                                 reset2 = null;
                             }, 540000);
                            var reset2 = setTimeout(function () {
                                 self.gameserver.pushBroadcast(new Message.chatResponse(self, "New Update Now! :)", Types.CHAT_TYPE.SYSTEM));
                                 reset2 = null;
                             }, 600000);
                            var reset2 = setTimeout(function () {
                                self.gameserver.forEachAccount(function (account) {
